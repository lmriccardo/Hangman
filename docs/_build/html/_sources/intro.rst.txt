...
+------+-----+--------+--------+--------+----+-------+-------+--------+
|      | N.  | Time x | Min    | Max    | P  | Min   | Max   | Hint   |
|      | Ro  | Round  | Word   | Word   | en | Hint  | Hint  | P      |
|      | und |        | Len    | Len    | al | N.    | N.    | enalty |
|      |     |        |        |        | ty |       |       |        |
+======+=====+========+========+========+====+=======+=======+========+
| Easy | 10  | 60 sec | 5      | 7      | 1  | 3     | 5     | 0      |
+------+-----+--------+--------+--------+----+-------+-------+--------+
| Me   | 10  | 300    | 7      | 9      | 1  | 2     | 4     | 0      |
| dium |     | sec    |        |        |    |       |       |        |
+------+-----+--------+--------+--------+----+-------+-------+--------+
| Hard | 10  | 600    | 9      | 11     | 1  | 1     | 3     | 0      |
|      |     | sec    |        |        |    |       |       |        |
+------+-----+--------+--------+--------+----+-------+-------+--------+
| Very | 10  | 1200   | 11     | 18     | 2  | 1     | 3     | 1      |
| Hard |     | sec    |        |        |    |       |       |        |
+------+-----+--------+--------+--------+----+-------+-------+--------+

..

   Note (HINT PENALTY): except for the first three modes, the VH-mode
   has the column *Hint Penalty* set to 1. What does It mean? It means
   that at the end of the round the total score would be 0 if the user
   has lost, otherwise is the 100 - (word_len / (max_hint \* 100)) \*
   number_used_hints \* hint_penalty. So, if *Hint penalty* is 0 then
   the score will remain the same (either 100 or 0).
...
All of them are installable via pip using :code:`pip install name==ver`, or directly using the requirements.txt file :code:`pip install -r requirements.txt`. If you are using Linux and pip is not available then use :code:`python3 -m pip install …`, otherwise first install pip using :code:`sudo apt-get install -y python3-pip` and then use the previous method.
...
Sadly, curses is not a module of the standard library of Python in Windows. In order to install it, you need to install NCurses and you can do it from `here <https://invisible-island.net/ncurses/>`__. Otherwise, you can try to install from MinGW: look at `this <https://e-l.unifi.it/pluginfile.php/805205/mod_resource/content/0/ncurses%20installation%20-%20en.pdf>`__. However, It really suggested to used a Unix-based OS.

Install ffmpeg
~~~~~~~~~~~~~~

In MacOS, it is really simple, just open a terminal and use sudo brew install ffmpeg. Otherwise, if you are in Linux first update the repo sudo apt-get update and then install the package :code:`sudo apt-get install -y ffmpeg`. In windows, please search over the internet (I’m sorry bro).

Run the game
------------

There are two ways to download this game. The first one is through pip module. Essentially, after done pip install hangman, **PLEASE MAKE SURE YOU ARE IN A FULL SCREEN TERMINAL** you can run it with :code:`python[ver] hangman`, where [ver] is the current version of python you are using to run the game. Another suggestion, if I can, is to create a python virtual environment, before running the game. It is very simple:

1. :code:`python[ver] -m venv venv_name`
2. :code:`source venv_name/bin/activate`
3. :code:`pip install hangman`
4. :code:`python hangman`

In Windows the second point change and become :code:`\venv_name\Scripts\activate`. Be careful, this may don’t work and return the error *cannot be loaded because the execution of scripts is disabled on this system*. In this situation there are two ways to solve the problem (both changing policy restriction). Open your windows PowersShell in the current folder where you want to execute the game and write one of the two commands:

1. :code:`Set-ExecutionPolicy Unrestricted -Scope Process`
2. :code:`Set-ExecutionPolicy Unrestricted -Force`
...
1. :code:`git clone -b cli_hangman https://github.com/lmriccardo/Hangman.git`
2. :code:`cd Hangman`
3. :code:`Create the virtual env and activate`
4. :code:`pip install -r requirements.txt`
5. :code:`python -m src.hangman`
...
.. |image0| image:: https://i.imgur.com/s7zZNQh.png
.. |image1| image:: https://imgur.com/cYASZsQ.png
